...
if ($result->num_rows > 0) {
    ...
    if ($room_type == 'laxuary single bed room') {
        $amount = $days_diff * 1500; // Luxury Single Bed Room rate
    } elseif ($room_type == 'laxuary double bed room') {
        $amount = $days_diff * 3000; // Luxury Double Bed Room rate
    } else {
...
<!-- JavaScript to prevent leaving without submission and delete booking if user navigates back -->
<script>
    ...
    window.addEventListener('beforeunload', function (event) {
        if (!isFormSubmitted) {
            // If the user tries to navigate away (like going back), delete the booking and confirm the navigation
            let xhr = new XMLHttpRequest();
            xhr.open('POST', 'delete_booking.php', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.send('email=<?php echo $email; ?>'); // Send the user's email to delete their booking
